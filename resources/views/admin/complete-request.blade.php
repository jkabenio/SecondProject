...
<div class="clearance_body">
    <h3 style="text-align:center; color:white; background-color: rgb(3, 3, 142); width: 100%"><b>All Completed Request</b></h3>
    @if (session('success'))
    ...
    @endif
    <form action="{{ route('admin.complete-request') }}" method="GET">
        {{csrf_field()}}
        @php
            $result = "";
            $id_val = 0;
        ...
        <div class="scroll_div">
            @foreach($complete_request as $index_count => $user)
                    <?php
                        $approve_count = 0;
                        $status_total = 0;
                    ?> 
